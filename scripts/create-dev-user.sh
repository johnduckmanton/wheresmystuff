...
# Create a user in the development Cognito User Pool
# Usage: ./scripts/create-dev-user.sh <email> <temporary-password>
...
echo -e "${GREEN}🔍 Creating user in development Cognito User Pool${NC}"
echo "=============================================="
...
if [ -z "$USER_POOL_ID" ] || [ "$USER_POOL_ID" = "None" ]; then
    ...
    echo "Trying with hardcoded dev User Pool ID from .env file..."
    USER_POOL_ID="eu-west-1_VM85YGyV9"
fi
...
if [ $? -eq 0 ]; then
    ...
    echo "3. The password must meet the development requirements:"
