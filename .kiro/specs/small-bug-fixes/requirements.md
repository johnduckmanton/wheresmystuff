# Requirements: Small Bug Fixes

## Overview

This spec addresses four small UI/UX bugs in the WheresMyStuff home inventory application that impact daily usability:

1. Entity lists (Things, Containers, Locations, People, Categories) do not default to showing most recently created items first
2. The mobile Thing detail view (ThingDetailSheet) provides no mechanism to delete photos
3. Success notifications after entity creation do not link to the newly created item
4. Items in the "Recently Added Things" list on the Home page dashboard are not clickable

All four bugs are isolated to the React frontend layer and require no backend changes.
